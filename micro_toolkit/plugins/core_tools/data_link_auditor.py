...
class DataLinkAuditorPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Data-Link Auditor")
        title.setStyleSheet("font-size: 26px; font-weight: 700; color: #10232c;")
        layout.addWidget(title)

        description = QLabel(
            "Audit filenames referenced in an Excel workbook against multiple source folders, then optionally move confirmed matches into a destination structure."
        )
    ...
    def _handle_result(self, payload: object) -> None:
        ...
        self.services.record_run(self.plugin_id, "SUCCESS", f"Audited workbook with {result['total_rows']} rows")

    def _handle_error(self, payload: object) -> None:
        message = payload.get("message", "Unknown data-link auditor error") if isinstance(payload, dict) else str(payload)
        self.output.setPlainText(message)
        self.summary_label.setText(message)
        self.services.record_run(self.plugin_id, "ERROR", message[:500])
        self.services.log("Data-Link Auditor failed.", "ERROR")
