...
class DeepScanAuditorPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Deep-Scan Auditor")
        title.setStyleSheet("font-size: 26px; font-weight: 700; color: #10232c;")
        layout.addWidget(title)

        description = QLabel(
            "Audit multiple folders for duplicate files or scan multiple Excel workbooks using per-file column names."
        )
        ...
        sources_title = QLabel("Selected Sources")
        ...
        self.browse_button = QPushButton("Add")
        ...
        self.created_checkbox = QCheckBox("Created Date")
        ...
        self.summary_label = QLabel("Choose one or more sources, then run the deep-scan audit.")
    ...
    def _update_mode_ui(self) -> None:
        ...
        self.browse_button.setText("Add Workbook(s)" if is_excel else "Add Folder")
        self.sources_table.clearContents()
        self.sources_table.setRowCount(0)
        if is_excel:
            self.sources_table.setColumnCount(2)
            self.sources_table.setHorizontalHeaderLabels(["Columns", "Workbook"])
            ...
            self.sources_table.setHorizontalHeaderLabels(["Folder"])
    ...
    def _run(self) -> None:
        ...
        if mode == "excel" and not excel_specs:
            QMessageBox.warning(self, "Missing Input", "Choose one or more workbooks.")
            return
        if mode == "folder" and not self._folder_sources:
            QMessageBox.warning(self, "Missing Input", "Choose one or more folders.")
            return
        ...
        self._table_model = None
        self.summary_label.setText("Running deep-scan audit...")
    ...
    def _handle_result(self, payload: object) -> None:
        ...
        self.summary_label.setText(
            f"Found {result['row_count']} duplicate result rows across {result['source_count']} "
            f"source entries in {result['mode']} mode. Previewing the first {len(result['dataframe'])} rows."
        )
        self.open_output_button.setEnabled(True)
        self.services.record_run(self.plugin_id, "SUCCESS", f"Generated deep-scan audit report in {result['mode']} mode")

    def _handle_error(self, payload: object) -> None:
        message = payload.get("message", "Unknown deep-scan auditor error") if isinstance(payload, dict) else str(payload)
        self.summary_label.setText(message)
        self.services.record_run(self.plugin_id, "ERROR", message[:500])
        self.services.log("Deep-Scan Auditor failed.", "ERROR")
