...
class WebScraperPage(QWidget):
    ...
    def _show_context_menu(self, position) -> None:
        ...
        row = self._selected_row()
        row_has_link = bool(row and str(row.get("link", "")).strip())
        row_enabled = row is not None
        items = [
            MenuActionSpec(
                label=self._pt("context.open_source", "Open Source URL"),
                callback=(lambda: QDesktopServices.openUrl(QUrl(str(row.get("source_url", ""))))) if row else None,
                enabled=row_enabled,
            ),
            MenuActionSpec(
                label=self._pt("context.open_page", "Open Page URL"),
                callback=(lambda: QDesktopServices.openUrl(QUrl(str(row.get("page_url", ""))))) if row else None,
                enabled=row_enabled,
            ),
            MenuActionSpec(
                label=self._pt("context.open_link", "Open Extracted Link"),
                callback=(lambda: QDesktopServices.openUrl(QUrl(str(row.get("link", ""))))) if row else None,
                enabled=row_has_link,
            ),
            MenuActionSpec(separator=True),
            MenuActionSpec(
                label=self._pt("context.copy_title", "Copy Title"),
                callback=(lambda: QGuiApplication.clipboard().setText(str(row.get("title", "")))) if row else None,
                enabled=row_enabled,
            ),
            MenuActionSpec(
                label=self._pt("context.copy_text", "Copy Text"),
                callback=(lambda: QGuiApplication.clipboard().setText(str(row.get("text", "")))) if row else None,
                enabled=row_enabled,
            ),
            MenuActionSpec(
                label=self._pt("context.copy_link", "Copy Link"),
                callback=(lambda: QGuiApplication.clipboard().setText(str(row.get("link", "")))) if row else None,
                enabled=row_has_link,
            ),
            MenuActionSpec(separator=True),
            MenuActionSpec(
                label=self._pt("context.export", "Export Visible Results"),
                callback=self._export_visible_results,
            ),
        ]
