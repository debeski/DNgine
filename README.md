...
| Ver. | Date | Highlights |
| --- | --- | --- |
| 0.8.1 | 2026-02-29 | **REBRANDED** from Micro-Toolkit to DNgine, published first beta release to PyPI, and github, Added built-in plugins manifest hash check on runtime, Fixed macOS dock showing a second app icon for the `Clip-Monitor` background process by suppressing the dock icon via `NSApplication` activation policy before the monitor's `QApplication` is created. |
