...
from dngine.core.fp_plugins import PACKAGE_GROUPS, catalog_entry_from_source, discover_package_source_manifests
...
def build_fp_plugins(
    *,
    ...
    for source in discover_package_source_manifests(ROOT / "fp_plugins"):
        if not source.plugins:
            continue
        category_label, category_label_ar = _group_labels(source.package_id)
        package_root = ROOT / "fp_plugins" / source.package_id
        ...
        catalog_entries.append(
            catalog_entry_from_source(
                source,
                download_url=(Path("..") / "dist" / "fp_plugins" / archive_path.name).as_posix(),
            )
        )
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", default=str(ROOT / "dist" / "fp_plugins"))
    parser.add_argument("--catalog-path", default=str(ROOT / "dngine" / "first_party_catalog.json"))
    parser.add_argument("--private-key", default=str(ROOT / "tools" / "fp_plugins_private_key.pem"))
