...
class CodeFactoryPage(QWidget):
    ...
    def _show_context_menu(self, position) -> None:
        ...
        if row is None:
            return
        menu = QMenu(self)
        open_file = menu.addAction(self._pt("context.open_file", "Open File"))
        open_folder = menu.addAction(self._pt("context.open_folder", "Open Containing Folder"))
        copy_before = menu.addAction(self._pt("context.copy_before", "Copy Before Preview"))
        copy_after = menu.addAction(self._pt("context.copy_after", "Copy After Preview"))
        if not row.get("before"):
            copy_before.setEnabled(False)
        if not row.get("after"):
            copy_after.setEnabled(False)
        ...
        elif chosen == copy_before:
            QGuiApplication.clipboard().setText(str(row.get("before", "")))
        elif chosen == copy_after:
            QGuiApplication.clipboard().setText(str(row.get("after", "")))
